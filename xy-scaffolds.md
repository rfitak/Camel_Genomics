...
# Repeat for Y chromosome
# The Y chromosome for cattle is in 12 scaffolds, concatenate them
cat <(echo ">Y") \
...
# Run the LASTZ shell script

#!/bin/bash -l
# author: rfitak
#SBATCH -J lastz-Y
#SBATCH -o lastz-Y.out
#SBATCH -e lastz-Y.err
#SBATCH -p hugemem
#SBATCH -n 1
#SBATCH --nodes=1
...
# This code will run lastz to align camel contigs to cow Y chromosome
# Cow Y is in Y.fasta
...
c=$(grep -c "$line" /wrk/rfitak/SEX_SCAFFOLDS/Y.Liu.aln.tab)
d=$(grep "$line" Y.Liu.merged.bed | perl -ne 'chomp;@a=split(/\t/,$_);$b=$a[2] - $a[1];print "$b\n"' | awk '{ sum+=$1} END {print sum}')
...
cat Y.cov | perl -ne '$a = () = $_ =~ /\t/g; if ($a < 5){print}' | cut -f1 > Y.remove.list
while read line
do
sed -i "/$line/d" Y.cov
done < Y.remove.list
...
# Build my Wilcox functions; test M ratio different from 0.5
...
# Build my Wilcox functions; test F ratio less than 0.5
